...
rss <- function(intercept, slope, x, y, data){
  resid <- pull(data, x) - (intercept + slope*pull(data, y))
  return(sum(resid^2))
}

slopes = seq(-0.2, 0.2, len = 100)
...
* If $f$ is not linear in $\boldsymbol\alpha \Rightarrow$ **nonlinear model**

  * For example, consider nonlinear parametric regression: $$ y_i = \frac{1}{1 + e^{\alpha_0 + \alpha_1 x_i}} + \varepsilon_i$$
    

* If $f$ is linear in $\boldsymbol\alpha \Rightarrow$ **linear model** 
...
|  | OLS  | limma |
| --------------- | :---------: | :------: |
| Estimated gene-wise <br>residual variance^[Not shown: estimation formulas for prior parameters $d_0$ and $s_0^2$]: | ${s}_g^2 =  \frac{1}{n-p} \hat{\boldsymbol\varepsilon}^T \hat{\boldsymbol\varepsilon}$ | $\tilde{s}^2_g = \frac{d_0s_0^2 + ds^2_g}{d_0 + d}$ |
