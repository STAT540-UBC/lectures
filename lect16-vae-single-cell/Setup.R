...
###############################################################
.matshow <- function(.mat, .lab=0, .axis.lab=0, .lw=0, .scale=TRUE, .limits=NULL) {
...
################################################################
## Torch tensor to matrix conversion
################################################################

tensor_to_matrix <- function(tensor) {
    ## Force use of torch package's as.array method
    ## by explicitly getting it from the namespace
    as_array_fn <- get("as.array.torch_tensor", envir = asNamespace("torch"))
    arr <- as_array_fn(tensor$to(device="cpu"))
    if(is.null(dim(arr)) || length(dim(arr)) == 1) {
        return(matrix(arr, ncol=1))
    }
    return(arr)
}

################################################################
## PanglaoDB and GSEA utilities
################################################################
