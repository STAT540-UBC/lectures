...
$$H_{0}:\, \boldsymbol{\theta}_{g} = 0\quad \textsf{vs.} \quad H_{1}:\, \boldsymbol{\theta}_{g} \neq 0$$

In a sense, it is the same as doing model comparison (after integrate out $\theta$):

$$H_{0}:\, \mathcal{N}\!\left(\mathbf{y}| \mathbf{0},\, \sigma^{2} I \right) \quad \textsf{vs.} \quad H_{1}:\,\mathcal{N}\!\left(\mathbf{y}\middle|\, \mathbf{0},\, \underbrace{\tau^{2} \mathbf{\color{magenta} K}}_{\textsf{\color{teal} e.g., local kinship}} + \sigma^{2} I\right)$$
...
## TWAS is a special case of Mendelian Randomization

\vfill

\centerline{\includegraphics[height=.3\textheight]{Vis/TWAS_MR_Egger/MR_model.pdf}}

\vfill

- TWAS is a gene-by-gene MR with no "pleiotropic" effect.

- But TWAS is initially designed to handle local LD structures

\vfill
\tiny
Burgess and Thompson, \emph{Eur J Epidemiol} (2018)

## TWAS may lead to a false conclusion 

- If there is a pleiotropic (direct) effect, not going through a risk factor of interest (e.g., gene)...
...
\onslide<1->{
\includegraphics[width=\linewidth]{Vis/TWAS_MR_Egger/IVW_simulated.pdf}
}
...
In a TWAS example:
\begin{itemize}
\item x-axis: eQTL effect sizes
\item y-axis: GWAS effect sizes
\end{itemize}

}

\only<2>{
\includegraphics[width=\linewidth]{Vis/TWAS_MR_Egger/IVW_urea_CAD.pdf}
}
...
## MR-Egger regression is meant to handle unidirectional pleiotropy
...
- Compare two types of genetic effect sizes

- Allow intercept in the scatter plot

:::
::: {.column width=.5}

\includegraphics[width=\linewidth]{Vis/TWAS_MR_Egger/MR_Egger_original.pdf}
...
(E.g., eQTL effects are not affected by unknown direct effects)
...
Bowden, Davey Smith, Burgess, \emph{Int J Epidemiol} (2015)

## Summary-based aggregation is an active research problem in MR research
...
\includegraphics[width=\linewidth]{Vis/TWAS_MR_Egger/MR_Egger_Mixture.pdf}
...
- We could have multiple regression lines in MR studies. 

- Recall a mixture of regression models.
...
Burgess, \textit{et al.}, \emph{Nature Communications} (2020)
