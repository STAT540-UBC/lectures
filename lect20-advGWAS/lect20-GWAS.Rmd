...
\item<3-> What is the variant of this variant?
...
## Estimating population admixture using top 10k variants
...
if.needed(out.file,
{
    ...
    xx <- torch_tensor(X$genotypes[,idx])
    ...
    etm$eval()
    out <- etm(xx)
    Z <- as.matrix(out$z.mean)

    H <- 
        apply(Z, 1, function(zz) { exp(zz - max(zz)) }) %>%
        apply(2, function(pp) { pp/sum(pp) })

    saveRDS(list(hidden=H, llik = llik.vec,
                 weight=as.matrix(etm$dec$lbeta)),
            file=out.file)
})
etm.out <- readRDS(out.file)
```

A potential generative model:
...
```{r fig.width=5.9, fig.height=1.2}
...
What is a generative model for a $\chi_{j}^{2} (\equiv Z_{j}^{2})$ statistics vector?
...
If we assume $\theta_{k} = \tau / p$, assuming all the variants can exert weak effects, and defining an LD score for a variant $j$, $l_{j} = \sum_{k} R^{2}_{jk}$, we have

$$\mathbb{E}\!\left[\chi^{2}_{j}\right] 
\only<1-2>{ = n \underbrace{\sum_{k} R_{jk}^{2}}_{\textsf{\color{blue} LD-score}} \mathbb{E}\!\left[ \theta_{k}^{2} \right] + 1 }
\only<2->{ = \underset{\textsf{\color{teal} sample size}}{n} \underset{\textsf{\color{blue} LD score}}{l_{j}} \underset{\textsf{\color{magenta}per SNP heritability}}{\frac{\tau}{p}} + 1}$$
\only<2>{
where $p$ is the total number of SNPs.
}

\onslide<3>{
We estimate the parameters by regressing the observed $\chi^{2}$ statistics on the reference LD scores $l_{j}$:
$$
\left(
\begin{array}{c}
\chi^{2}_{1} \\
...
\end{array} 
\right)
\underset{\textsf{\color{magenta}per SNP heritability}}{\frac{\tau}{p}}
...
\centerline{\includegraphics[height=.7\textheight]{Vis/sLDSC_GTEx.pdf}}
