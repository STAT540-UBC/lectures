...
.file.name <- "GSE85241/GSE85241_cellsystems_dataset_4donors_updated.csv.gz"
if(!file.exists(.file.name)){
    getGEOSuppFiles("GSE85241")
}

mtx.data <- fileset.list("Data/GSE85241")
if(!all(file.exists(unlist(mtx.data)))){
    raw.data <- fread(.file.name)
    .dat <- as.matrix(raw.data[, -1])
    .mtx <- Matrix::Matrix(.dat, sparse=TRUE)
    write.sparse(.mtx, unlist(raw.data[, 1]), colnames(.dat), "Data/GSE85241")
}
.info <- rcpp_mmutil_info(mtx.data$mtx)
```
...
```{r}
eda.out <- run.eda(mtx.data$mtx, mtx.data$row, mtx.data$col,
...
if.needed(.file,
{
    .scores <- rcpp_mmutil_compute_scores(mtx_file = mtx.data$mtx,
...
{
    selected.data <-
        rcpp_mmutil_copy_selected_rows(mtx.data$mtx,
                                       mtx.data$row,
                                       mtx.data$col,
                                       .selected,
                                       .selected.hdr)
...
$$\min D_{\textsf{KL}}\left(p_{ij}\| q_{ij}\right)=\sum_{ij} p_{ij} \log \frac{p_{ij}}{q_{ij}}$$
}
...
\vfill
\centerline{\includegraphics[width=\textwidth]{Vis/doublet_knn.pdf}}
\vfill
\tiny
McGinnis \textit{et al.}, \emph{Cell Systems} (2019)
...
{
  .out <-
    rcpp_mmutil_match_files(mtx.data$mtx,
                            doublet.data$mtx,
                            knn = 50,
                            RANK = 50,
                            NUM_THREADS = 15)
...
```{r}
.src.cells <-
  fread(mtx.data$col, col.names="cell", header=FALSE) %>%
...
\vfill
\centerline{\includegraphics[width=\textwidth]{Vis/doublet_ann.pdf}}
\vfill
\tiny
Bernstein \textit{et al.}, \emph{Cell Systems} (2020)
...
```{r fig.width=2.5, fig.height=2, only.plot=3}
pred.dt <-
    data.table(`name` = .readv(mtx.data$col),
               `doublet` = y) %>%
...
\centerline{\includegraphics[width=\textwidth]{Vis/scRNA_normalization/scanorama.pdf}}
...
mtx.data <- fileset.list("Data/GSE85241")
.info <- rcpp_mmutil_info(mtx.data$mtx)
cell.index <- rcpp_mmutil_read_index(mtx.data$idx)
```
...
.dt <-
    last(etm.result$hh) %>%
    as.data.table() %>% 
    mutate(`name` = .readv(mtx.data$col)) %>% 
