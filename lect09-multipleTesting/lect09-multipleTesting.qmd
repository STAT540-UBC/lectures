...
|       $d$       |             residual degrees of freedom $(n-p)$              | `gFit$df.residual` |        $39-4=35$[^1]         | $\checkmark$  |
...
| $\tilde{s}_g^2$ | posterior mean of $s_g^2$ (moderated resid. var.) |  `ebFit$s2.post`   |         45K numbers          |               |
...
## `topTable()` extracts output in a convenient format^[this list is truncated - see `?topTable` for a full listing]!
...
1. For **large** variances, limma \_\_\_\_\_\_ the gene-specific variance estimates.
    (a) Increases
    (b) Decreases 
  
2. For **small** variances, limma \_\_\_\_\_\_ the gene-specific variance estimates.
    (a) Increases
    (b) Decreases 
::: 
...
:::{.fragment .fade-in}
| Rank $(j)$ | P-val | (j/m)\*q | Reject $H_0$? | $FWER < 0.05$? |
|:---:|---|---|:----:|:---:|
