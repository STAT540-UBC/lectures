...
* If $n \to \infty$, how confident are you about the estimated parameter $\hat{\theta}$?
...
* If so, how "frequently" the constructed CI will include the true parameter as $n \to \infty$? 
...
* How likely is this new observation $x^\star$ given the other observed data points?
...
$$p\left(\theta \in (\hat{\theta}_{n} - 2\hat{\textsf{se}}_{n}, \hat{\theta}_{n} + 2\hat{\textsf{se}}_{n}) \right)$$


:::
::: {.column width=.5}

$$p\left(\theta | \{i \in [n]:\, x_{i}\} \right)$$
...
\only<1>{
$$p({\color{magenta} \underbrace{x^{\star}}_{\textsf{future}}} | {\color{teal} \underbrace{\{x_{i}\}}_{\textsf{data}}})  = \int d\mu\, d\gamma\, 
\overbrace{
p( \underbrace{ {\color{magenta} x}^{\star} }_{\textsf{new observation} } \, | \mu,\gamma) 
}^{\textsf{data generating}}
\, \, 
\overbrace{
p(\mu, \gamma \, |  \underbrace{ {\color{teal} \{x_{i}\}} }_{\textsf{training data}} )
}^{\textsf{posterior probability}}$$
}

\only<2>{
$$p({\color{magenta} \underbrace{x^{\star}}_{\textsf{future}}} | {\color{teal} \underbrace{\{x_{i}\}}_{\textsf{data}}})  = \int d\mu\, d\gamma\, p( {\color{magenta} x}^{\star}  \, | \underbrace{ \mu,\gamma) \, \, p(\mu, \gamma}_{\textsf{averaging over uncertainty}} \, |  {\color{teal} \{x_{i}\}} )$$
}
...
## _Side note:_ Bayesian inference often involves heavy computational work
...
* Where is Markov Chain? 

* Why is it Monte Carlo?
...
$$\alpha, \beta_{j} \sim \mathcal{N}\!\left(0, 1\right)$$

$$\phi \sim \operatorname{Gamma}(1, 1)$$

$$\lambda_{i} | \phi \sim \operatorname{Gamma}(\phi, 1)$$

$$\eta_{i} \gets \mathbf{x}_{i} \boldsymbol{\beta} + \alpha$$

$$Y_{i} | \eta_{i}, \lambda_{i} \sim \operatorname{Poisson}(\lambda_{i} \exp(\eta_{i}))$$
