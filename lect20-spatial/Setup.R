...
.sort.matrix <- function(.X) {
    as.matrix(.X) %>%
        reshape2::melt() %>%
        dplyr::rename(row = Var1, col = Var2, weight = value) %>%
...
.gg.plot <- function(...) {
    ggplot(...) +
        ...
        theme(legend.spacing = unit(.1, "lines"),
              ...
              legend.background = element_rect(fill='transparent', linewidth=0.05),
    ...
    knitr::knit_hooks$set(plot = function(x, options) {
        if (!is.null(options$onslide.plot)) {
            ...
            ret <- paste(c(bf, knitr::hook_plot_tex(x, options), "}"),
                         collapse = "\n")
            return(ret)
        } else if (!is.null(options$overlay.plot)) {
            i <- options$fig.cur
            hand <- as.numeric(ifelse(i == options$overlay.plot, 1, 0))
            bf <- paste0("\\only<", i, "| handout:", hand, ">{")
            ret <- paste(c(bf, knitr::hook_plot_tex(x, options), "}"), collapse = "\n")
            return(ret)
        } 
