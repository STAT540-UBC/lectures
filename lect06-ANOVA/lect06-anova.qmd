...
[^1]: Note differences in sign between t-test & linear regression: pay attention to which group is 'reference'
...
[^2]: Note that the t-statistic squared is equal to the ANOVA F statistic

## *t*-test vs *line*ar regression: where's the *line*[^3]?
...
[^3]: Note that the $x$-axis in these plots is not numerical, thus a line in this space does not have any mathematical meaning. 
...
* A natural *estimator* of the population mean $\mu_j$ is the **sample** mean $\hat{\mu}_j = \bar{Y} = \frac{\sum_{i=1}^{n_j}Y_{ij}}{n_j}$
...
**1-way ANOVA with many levels**[^4] using a linear model - e.g for 3 groups:
...
[^4]: in general; yet *another* parameterization can be used to present ANOVA 
...
## Linear[^1] regression can include *quantitative* & *qualitative* covariates 


![](img/LM_vbles.png){fig-align="center"}

[^1]: Here we mean **linear** in the parameters $\boldsymbol{\alpha}$; $X$ can contain $x^2$, $log(x)$, etc
...
By default, R uses the reference-treatment parameterization[^1] 

[^1]: but [you can change that](http://genomicsclass.github.io/book/pages/expressing_design_formula.html)!
