...
# Generate elliptical contours for RSS (centered at OLS solution)
...
# Create contour data for RSS
make_ellipse <- function(a, b, cx, cy) {
  data.frame(
    theta1 = cx + a * cos(theta),
    theta2 = cy + b * sin(theta)
  )
}

contours <- rbind(
  make_ellipse(0.3, 0.2, ols_theta1, ols_theta2) %>% mutate(level = "1"),
  make_ellipse(0.6, 0.4, ols_theta1, ols_theta2) %>% mutate(level = "2"),
  make_ellipse(0.9, 0.6, ols_theta1, ols_theta2) %>% mutate(level = "3")
)

# L2 constraint (circle)
l2_radius <- 0.8
l2_constraint <- data.frame(
  theta1 = l2_radius * cos(theta),
  theta2 = l2_radius * sin(theta)
)
l2_solution <- data.frame(theta1 = 0.64, theta2 = 0.48)

# L1 constraint (diamond)
l1_lambda <- 1.0
l1_constraint <- data.frame(
  theta1 = c(l1_lambda, 0, -l1_lambda, 0, l1_lambda),
  theta2 = c(0, l1_lambda, 0, -l1_lambda, 0)
)
l1_solution <- data.frame(theta1 = 1.0, theta2 = 0)

# L0 constraint (SuSiE: axes with sum-to-one constraint)
l0_lambda <- 1.0
l0_constraint <- data.frame(
  theta1 = c(-l0_lambda, l0_lambda, NA, 0, 0),
  theta2 = c(0, 0, NA, -l0_lambda, l0_lambda)
)
l0_solution <- data.frame(theta1 = 1.0, theta2 = 0)

# L2 penalty plot
p1 <- ggplot() +
  geom_path(data = contours, aes(theta1, theta2, group = level),
            color = "steelblue", linewidth = 0.8, alpha = 0.6) +
  geom_polygon(data = l2_constraint, aes(theta1, theta2),
               fill = "coral", alpha = 0.2, color = "coral", linewidth = 1) +
  geom_point(data = l2_solution, aes(theta1, theta2),
             color = "red", size = 3) +
  geom_point(aes(ols_theta1, ols_theta2), color = "blue", size = 3, shape = 4, stroke = 2) +
  coord_fixed(xlim = c(-1.5, 1.5), ylim = c(-1.5, 1.5)) +
  labs(x = expression(theta[1]), y = expression(theta[2]),
       title = bquote(L[2]~"(Ridge): "~sum(theta[j]^2) < lambda)) +
...
p2 <- ggplot() +
  geom_path(data = contours, aes(theta1, theta2, group = level),
            color = "steelblue", linewidth = 0.8, alpha = 0.6) +
  geom_polygon(data = l1_constraint, aes(theta1, theta2),
               fill = "coral", alpha = 0.2, color = "coral", linewidth = 1) +
  geom_point(data = l1_solution, aes(theta1, theta2),
             color = "red", size = 3) +
  geom_point(aes(ols_theta1, ols_theta2), color = "blue", size = 3, shape = 4, stroke = 2) +
  coord_fixed(xlim = c(-1.5, 1.5), ylim = c(-1.5, 1.5)) +
  labs(x = expression(theta[1]), y = expression(theta[2]),
       title = bquote(L[1]~"(LASSO): "~sum(abs(theta[j])) < lambda)) +
  theme_minimal(base_size = 12) +
  theme(plot.title = element_text(hjust = 0.5, size = 9))

# L0 penalty plot (SuSiE)
p3 <- ggplot() +
  geom_path(data = contours, aes(theta1, theta2, group = level),
            color = "steelblue", linewidth = 0.8, alpha = 0.6) +
  geom_path(data = l0_constraint, aes(theta1, theta2),
            color = "coral", linewidth = 2) +
  geom_point(data = l0_solution, aes(theta1, theta2),
             color = "red", size = 3) +
  geom_point(aes(ols_theta1, ols_theta2), color = "blue", size = 3, shape = 4, stroke = 2) +
  coord_fixed(xlim = c(-1.5, 1.5), ylim = c(-1.5, 1.5)) +
  labs(x = expression(theta[1]), y = expression(theta[2]),
       title = bquote(L[0]~"(SuSiE): "~sum(I(theta[j] != 0)) == 1)) +
  theme_minimal(base_size = 12) +
  theme(plot.title = element_text(hjust = 0.5, size = 9))
...
```{r fig.width=2, fig.height=2.8, echo=FALSE, onslide.plot="1-"}
p1
```
...
```{r fig.width=2, fig.height=2.8, echo=FALSE, onslide.plot="2-"}
p2
```
...
```{r fig.width=2, fig.height=2.8, echo=FALSE, onslide.plot="3"}
p3
```
